let setParam = JSON.parse(JSON.stringify(localStorage.getItem("setParam") || "{}"));
let data = {};
console.log("init data.js");
data = JSON.parse(setParam);

/*고객정보 데이터*/
if(data.userData === null || data.userData === undefined){
    console.log("wasdasdasds")
    data.userData = [];
    localStorage.setItem("setParam",JSON.stringify(data));
}

/*장바구니 데이터*/
if(data.basket === null || data.basket === undefined){
    data.basket = [];
    localStorage.setItem("setParam",JSON.stringify(data));
}

/*크루모집 데이터*/
if(data.cruTable === null || data.cruTable === undefined){
    data.cruTable = [];
    localStorage.setItem("setParam",JSON.stringify(data));
}

/*공지 데이터*/
if(data.boardTable === null || data.boardTable === undefined){
    data.boardTable = [];
    localStorage.setItem("setParam",JSON.stringify(data));
}

/*메뉴 데이터*/
if(data.menuTable === null || data.menuTable === undefined){
    data.menuTable = [
        {
            mnuId: 1001,
            mnuNa: "에그마요 샌드위치",
...
setParam = JSON.parse(localStorage.getItem("setParam"));
